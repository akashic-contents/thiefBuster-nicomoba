...
export namespace audioUtil {
	...
	export function inUse(_soundId: string): boolean {
		...
		const asset = g.game.scene().asset.getAudioById(_soundId);
		if (!asset) {
			console.error("AudioUtil.inUse: not found " + _soundId + " in asset.");
			return false;
		}
		return asset.inUse();
	}
	...
	export function play(_soundId: string): g.AudioPlayer {
		...
		const asset = g.game.scene().asset.getAudioById(_soundId);
		if (!asset) {
			console.error("AudioUtil.play: not found " + _soundId + " in asset.");
			return null;
		}
	...
	export function stop(_soundId: string): void {
		...
		const asset = g.game.scene().asset.getAudioById(_soundId);
		if (!asset) {
			console.error("AudioUtil.stop: not found " + _soundId + " in asset.");
			return;
		}
		const info = getPlayingAudioInfo(asset);
		if (info !== null) {
	...
	export function getDuration(_soundId: string): number {
		...
		const asset = g.game.scene().asset.getAudioById(_soundId);
		if (!asset) {
			console.error("AudioUtil.getDuration: not found " + _soundId + " in asset.");
			return 0;
		}
	...
	export function isPlaying(_soundId: string): boolean {
		...
		const asset = g.game.scene().asset.getAudioById(_soundId);
		if (!asset) {
			console.error("AudioUtil.isPlaying: not found " + _soundId + " in asset.");
			return false;
		}
		const info = getPlayingAudioInfo(asset);
		if (info === null) {
